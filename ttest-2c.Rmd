...
if (et) se <- sqrt(s1^2/n1+s^2/n2) else se <- sqrt(sp^2/n1+sp^2/n2)
tobs <- (mu1-mu2)/se
...
$$
  S^2_\text{`r a`}=\frac{n_\text{`r a`}}{n_\text{`r a`}-1}\hat\sigma_\text{`r a`}=\frac{`r n1`}{`r n1`-1}`r sh1^2=`r s1` \qquad`
  S^2_\text{`r b`}=\frac{n_\text{`r b`}}{n_\text{`r b`}-1}\hat\sigma_\text{`r b`}=\frac{`r n2`}{`r n2`-1}`r s2^2
$$
","
...
$$
  ...
  \frac{`r n1`\cdot`r sh1^2`+`r n2`\cdot`r sh2^2`}{`r n1`+`r n2`-2}=`r s2p`
$$
")`

- 
\begin{eqnarray*}
 \frac{\hat\mu_\text{`r a`} - \hat\mu_\text{`r a`}} 
 {\sqrt{\frac {S^2_{`r q1`}}{n_\text{`r a`}}+\frac {S^2_{`r q2`}}{n_\text{`r b`}}}}&\sim&t_{n_\text{`r a`}+n_\text{`r b`}-2}\\
    t_{\text{obs}}
&=& \frac{ (`r mu1`- `r mu2`)} {\sqrt{\frac{`r s2f1`}{n1}+\frac{`r s2f2`}{n2}}}
 =  `r tobs`\, .
\end{eqnarray*}
...
- CONCLUSIONE: i dati `r ifelse(tobs<tc & h1=="<", " **non** sono", "**sono**")` coerenti con $H_{0}$ al LdS del `r ifelse(H1=="\\neq",2*alpha*100,alpha*100)`%
